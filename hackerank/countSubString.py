...
while start <= len(string) - len(sub_string): # loop should run according to the length of substring and string, if substring is only 3charcahracer, then we dont need to run the loop at the whole lenfth of the string to find the occuurence of sdubstring, as after 4th position, there will be only2 characters
